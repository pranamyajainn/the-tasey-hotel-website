...
export default function BookingModal({ isOpen, onClose, initialData }: BookingModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 overflow-y-auto animate-in fade-in duration-300">
      <div className="relative w-full max-w-2xl bg-[#FFFDF9] rounded-3xl border border-[#C5A059]/40 shadow-2xl overflow-hidden my-auto text-[#1C1815]">
        {/* Modal Header */}
        <div className="p-6 bg-[#F8F3EA] border-b border-[#E5DCCB] flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#C5A059] to-[#8C6310] p-0.5">
              <div className="w-full h-full bg-[#FFFDF9] rounded-full flex items-center justify-center text-[#8C6310] font-serif font-bold text-lg">
                T
              </div>
            </div>
            <div>
              ...
              <p className="text-[11px] text-[#8C6310] font-bold">
          ...
          <button
            ...
            className="p-2 rounded-full text-[#5C5248] hover:text-[#1C1815] hover:bg-[#E5DCCB]/50 transition-colors"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        {!confirmed ? (
          <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6">
            {/* Category Choice */}
            <div>
              <label className="text-xs uppercase font-bold tracking-wider text-[#8C6310] block mb-2">
                Select Experience / Category
              </label>
              <select
                value={category}
                onChange={(e) => setCategory(e.target.value)}
                className="w-full bg-[#F8F3EA] border border-[#E5DCCB] rounded-xl px-4 py-3 text-sm text-[#1C1815] font-semibold focus:outline-none focus:border-[#C5A059] cursor-pointer"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="text-xs uppercase font-semibold tracking-wider text-[#5C5248] block mb-1.5 flex items-center gap-1">
                ...
                <input
                  ...
                  className="w-full bg-[#F8F3EA] border border-[#E5DCCB] rounded-xl px-3 py-2.5 text-xs text-[#1C1815] font-medium focus:outline-none focus:border-[#C5A059]"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-[#F8F3EA] border border-[#E5DCCB] rounded-xl px-3 py-2.5 text-xs text-[#1C1815] font-medium focus:outline-none focus:border-[#C5A059] cursor-pointer"
                >
            ...
            <div className="space-y-4 pt-2 border-t border-[#E5DCCB]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-[#5C5248] block mb-1 font-semibold">Full Name *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-xs text-[#5C5248] block mb-1 font-semibold">Phone Number (WhatsApp) *</label>
                  <input
                    type="tel"
                    required
              ...
              <div>
                <label className="text-xs text-[#5C5248] block mb-1 font-semibold">Email Address</label>
                <input
                  type="email"
              ...
              <div>
                <label className="text-xs text-[#5C5248] block mb-1 font-semibold">Special Requests & Excursion Preferences</label>
                <textarea
                  rows={2}
                  placeholder="e.g., Elephant feeding slot preference, high-floor hill view room..."
                  value={specialRequests}
                  onChange={(e) => setSpecialRequests(e.target.value)}
                  className="w-full bg-[#F8F3EA] border border-[#E5DCCB] rounded-xl px-3 py-2.5 text-xs text-[#1C1815] font-medium focus:outline-none focus:border-[#C5A059] resize-none"
                />
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-[#E5DCCB] flex items-center gap-3">
              <button
                type="button"
                onClick={resetAndClose}
                className="px-5 py-3 rounded-xl border border-[#E5DCCB] text-xs text-[#5C5248] hover:text-[#1C1815] hover:bg-[#F8F3EA] font-semibold"
              >
                Cancel
              </button>
              <button
          ...
          <div className="p-8 text-center space-y-6 animate-in zoom-in-95 duration-300">
            <div className="w-16 h-16 rounded-full bg-[#F8F3EA] border border-[#C5A059]/40 p-1 mx-auto shadow-md">
              <div className="w-full h-full bg-[#C5A059] rounded-full flex items-center justify-center text-white">
            ...
            <div className="bg-[#F8F3EA] p-6 rounded-2xl border border-[#C5A059]/40 max-w-md mx-auto text-left space-y-3 shadow-inner">
              <div className="flex justify-between items-center pb-3 border-b border-[#E5DCCB]">
                <span className="text-xs text-[#5C5248] uppercase tracking-wider font-semibold">Reference ID</span>
                <button
                  onClick={handleCopyId}
                  className="flex items-center gap-1.5 text-xs font-mono font-bold text-[#8C6310] hover:underline"
                >
                  <span>{bookingId}</span>
                  {copied ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-[#C5A059]" />}
                </button>
              </div>

              <div className="grid grid-cols-2 gap-3 text-xs">
                <div>
                  <span className="text-[#5C5248] block text-[10px]">Category</span>
                  <span className="text-[#1C1815] font-bold">{category}</span>
                </div>
                <div>
                  <span className="text-[#5C5248] block text-[10px]">Guests</span>
                  <span className="text-[#1C1815] font-bold">{guestsCount}</span>
                </div>
                <div>
                  <span className="text-[#5C5248] block text-[10px]">Check-In / Date</span>
                  <span className="text-[#1C1815] font-bold">{checkIn}</span>
                </div>
                <div>
                  <span className="text-[#5C5248] block text-[10px]">Check-Out</span>
            ...
            <div className="pt-4 flex flex-col sm:flex-row justify-center gap-3">
              ...
              <button
                onClick={resetAndClose}
                className="px-6 py-3 rounded-xl bg-[#F8F3EA] hover:bg-[#C5A059] hover:text-white text-[#1C1815] font-bold text-xs uppercase tracking-wider transition-colors border border-[#C5A059]/40"
              >
                Close & Return
              </button>
            </div>
