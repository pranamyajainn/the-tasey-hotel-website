...
export default function Rooms({ onOpenBooking }: RoomsProps) {
  ...
  const rooms = [
    ...
    },
    {
      ...
      desc: "Designed for discerning travelers, the Executive Room features a private sit-out balcony overlooking the Aravali range, a plush workspace lounge, espresso maker, and rich teak wood accents.",
      amenities: [
    ...
    },
    {
      id: "suite-room",
      name: "Suite Room",
      tagline: "Royal Penthouse Sanctuary with 360° Aravali Vista",
      image: "/images/suite-room.jpg",
      price: "₹12,999",
      per: "night + taxes",
      badge: "Ultimate Royal Luxury",
      size: "700 sq.ft",
      view: "360° Aravali Mountain & Fort View",
  ...
  return (
    <section id="rooms" className="py-24 relative bg-[#F8F3EA] border-t border-b border-[#C5A059]/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="mt-4 text-[#5C5046] text-sm sm:text-base font-normal">
            Every room at The Tasey is thoughtfully curated with modern luxury amenities, warm lighting, and captivating Aravali hill views.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 sm:gap-4 mb-12">
          {rooms.map((room, idx) => (
            <button
              ...
              className={`px-6 py-3.5 rounded-2xl text-sm font-bold tracking-wider transition-all duration-300 flex items-center gap-2 border ${
                activeRoomIndex === idx
            ...
            >
              <Bed className={`w-4 h-4 ${activeRoomIndex === idx ? "text-[#C5A059]" : "text-[#B88E36]"}`} />
              <span>{room.name}</span>
            </button>
          ))}
        </div>
        ...
        <div className="bg-[#FFFDF9] rounded-3xl overflow-hidden border border-[#C5A059]/35 shadow-xl grid grid-cols-1 lg:grid-cols-12 gap-0">
          {/* Room Image Display */}
          <div className="lg:col-span-7 relative min-h-[360px] sm:min-h-[460px] group overflow-hidden">
            <Image
            ...
            <div className="absolute top-6 left-6 flex items-center gap-2">
              <span className="px-3.5 py-1.5 rounded-full bg-[#FFFDF9]/90 backdrop-blur-md border border-[#C5A059]/40 text-[#9E7B32] text-xs font-bold uppercase tracking-wider shadow-md">
                {activeRoom.badge}
              </span>
            </div>

            {/* Room Quick Specs Strip */}
            <div className="absolute bottom-6 left-6 right-6 flex flex-wrap gap-4 text-xs text-white bg-black/60 backdrop-blur-md p-4 rounded-xl border border-white/20">
          ...
          <div className="lg:col-span-5 p-8 sm:p-10 flex flex-col justify-between space-y-6">
            <div>
              <div className="flex justify-between items-baseline mb-2">
                ...
                <div className="text-right">
                  <span className="font-serif-luxury text-2xl sm:text-3xl font-bold text-[#9E7B32]">
                    {activeRoom.price}
                  </span>
                  <span className="text-[11px] text-[#6E645A] block font-medium">{activeRoom.per}</span>
                </div>
              </div>

              <p className="text-xs uppercase tracking-widest text-[#B88E36] font-bold mb-4">
                {activeRoom.tagline}
              </p>
              ...
              <ul className="space-y-2.5">
                {activeRoom.amenities.map((amenity, i) => (
                  <li key={i} className="flex items-start gap-2.5 text-xs text-[#3D352E] font-medium">
                    <CheckCircle2 className="w-4 h-4 text-[#B88E36] shrink-0 mt-0.5" />
                    <span>{amenity}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="pt-6 border-t border-[#C5A059]/20 flex flex-col sm:flex-row gap-3">
              <button
                onClick={() => onOpenBooking(activeRoom.name)}
                className="flex-1 py-3.5 rounded-xl bg-[#1C1815] text-[#FDFBF7] font-bold text-xs sm:text-sm tracking-wider uppercase shadow-md hover:bg-[#B88E36] hover:text-white transition-all flex items-center justify-center gap-2"
              >
